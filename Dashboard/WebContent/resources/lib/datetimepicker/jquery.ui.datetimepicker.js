...
(function($) { // hide the namespace
    ...
    $.extend(Datetimepicker.prototype, {
        ...
        _doKeyDown: function(event) {
            ...
            if ($.datetimepicker._datepickerShowing)
                ...
                case 13: var sel = $('td.' + $.datetimepicker._dayOverClass, inst.dpDiv).
                    ...
                    if (sel[0])
                        $.datetimepicker._selectDay(event.target, inst.selectedMonth, inst.selectedYear, sel[0], inst.currentHour, inst.currentMinute, inst.currentSecond, inst.currentAMPM);
                    else
                        $.datetimepicker._hideDatepicker();
        ...
        */
        ...
        /* Changes the Second hand to the correct image.
        t: the Clock instance Div
        Second: the Second to set it to.
        */
        DrawSeconds: function(t, Second) {
            var objID = t.id.split("_");
            var ID = objID[objID.length - 1];
            var Seconds = parseInt(Second).toString();
            $("#DP_jQuery_Second_" + ID).val(parseInt(Second).toString());

            /* I'm lost as to why parseInt can't parse 08 as 8 or 09 as 9 but all the others */
            if (Second == '08')
                $("#DP_jQuery_Second_" + ID).val('8');
            if (Second == '09')
                $("#DP_jQuery_Second_" + ID).val('9');
                
            var ImageIndex = Second
            if (ImageIndex > 30) {
                ImageIndex = ImageIndex - 60;
                ImageIndex = Math.abs(ImageIndex);
                t.children[0].style.backgroundPosition = '-269px -' + (ImageIndex * 269) + 'px';

...
// 矢印の終点を終了時刻を示すようにする。
infinispan.arrowState = {};
infinispan.arrowState.xOffset = infinispan.customTriangle.width;
infinispan.arrowState.changableFlag = true;
infinispan.arrowState.minLength = 30;

infinispan.ArrowStateElementView = Backbone.View
		.extend({
			...
			render : function() {
				var rate = 1;
				if (this.model.attributes.width < infinispan.arrowState.minLength) {
					rate = this.model.attributes.width * 1.0
							/ infinispan.arrowState.minLength;
					if (rate < 0.7)
						rate = 0.7;
				} else {
					this.model.attributes.width = this.model.attributes.width
							- infinispan.arrowState.xOffset;
				}
				...
				this.element[1] = new infinispan.customtriangle(
						triangleData.attributes, this._paper, rate);
			...
			addMouseoutArrow : function() {
				infinispan.arrowChart.detailInfoElement.animationDisappear({
					text : ""
				});

				if (infinispan.arrowState.changableFlag) {
			...
			addMouseoverArrow : function(event) {
				infinispan.arrowChart.detailInfoElement.animationAppear({
					text : this.taskInfo.TaskAttemptID + "\n"
							+ this.taskInfo.Status + ":  @"
							+ this.taskInfo.Hostname,
					"event" : event
				});
				if (infinispan.arrowState.changableFlag) {
					...
					if ($.isArray(arrowElement)) {
						...
						infoString = " ID:</br>" + attemptIDArray[0]
								+ "</br>" + "_" + attemptIDArray[1] + "</br>"
								+ "_" + attemptIDArray[2] + "_"
								+ attemptIDArray[3] + "</br>" + "_"
								+ attemptIDArray[4] + "_" + attemptIDArray[5]
								+ "</br>" + "</br>" + "Status:</br>"
								+ arrow.Status + "</br>" + "</br>" + startd
								+ " - </br>" + find + "</br>" + "</br>"
						...
						infoString = "<p>JobId:</br>" + arrow.JobID
								+ "</br>" + "TaskAttemptID:</br>"
								+ arrow.TaskAttemptID + "</br>"
								+ "Status:</br>" + arrow.Status + "</br>"
								+ "StartTime:</br>" + startd + "</br>"
								+ "FinishTime:</br>" + find + "</br>"
								+ "Hostname:</br>" + arrow.Hostname
								+ "</br></p>";
			...
			addMouseClickArrow : function(event) {
				if (infinispan.arrowState.changableFlag) {
					infinispan.arrowState.changableFlag = false;
					$("#taskInfoSpace").css({
						"border-style" : "inset"
					});
				} else {
					...
					var infoString = " ID:</br>" + attemptIDArray[0] + "</br>"
							...
							+ attemptIDArray[2] + "_" + attemptIDArray[3]
							+ "</br>" + "_" + attemptIDArray[4] + "_"
							+ attemptIDArray[5] + "</br>" + "</br>"

...
public class ResponseTimeSummaryRecordAccessor
{
		...
		Timestamp startTime, Timestamp endTime) throws SQLException
	{
		...
		exceptionValues = ReportDao.selectSumAll(database, startTime, endTime,
		...
		for (Map.Entry<String, List<ReportItemValue>> entry : cntValues.entrySet())
		{
			for (int cnt = 0; cnt < entry.getValue().size(); cnt++)
			{
				...
				List<ReportItemValue> exceptionList = exceptionValues.get(itemName);
				...
				record = updateResponseTimeSummaryRecord(record, minValue, maxValue, cntValue,
	...
	private ResponseTimeSummaryRecord updateResponseTimeSummaryRecord(
		ResponseTimeSummaryRecord record, ReportItemValue minValue, ReportItemValue maxValue,
		ReportItemValue cntValue, ReportItemValue aveValue, ReportItemValue exceptionValue,
		ReportItemValue stallValue)
	{
		if (record == null)
		{
			record = new ResponseTimeSummaryRecord();
